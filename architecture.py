...
def build_conv_cascade(filters: int, dropout: float, activation: str, kernel_initializer: str) -> tf.keras.Sequential:
...
def build_reduction_sequential(filters: int, dropout: float, activation: str, kernel_initializer: str, prev_node: tf.keras.layers.Layer) -> tuple[tf.keras.Sequential, InceptionModule]:

    seq = build_conv_cascade(filters, dropout, activation, kernel_initializer)(prev_node)
    inception = InceptionModule(filters, 2, activation, kernel_initializer)(seq)

    return seq, inception

def build_expansion_sequential(filters: int, dropout: float, activation: str, kernel_initializer: str, prev_node: tf.keras.layers.Layer, concat_with: tf.keras.Sequential) -> tf.keras.Sequential:
...
def build_model(input_shape: tuple[int, int, int], num_classes: int, activation: str, kernel_initializer: str) -> tf.keras.Model:
    ...
    e = build_expansion_sequential(128, 0.2, activation, kernel_initializer, r5, r4)

    e = build_expansion_sequential(64, 0.2, activation, kernel_initializer, e, r3)

    e = build_expansion_sequential(32, 0.2, activation, kernel_initializer, e, r2)
